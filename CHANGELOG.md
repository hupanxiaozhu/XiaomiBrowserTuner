...
- **「主页滚动回弹」**（默认关闭）：去掉简洁版主页滚到顶部 / 底部时的回弹（弹性过冲）动画，
  同时不再出现「滚到底部被强制显示搜索栏」。

  这两件事在宿主里是**同一套逻辑** —— 回弹把搜索栏带了出来。简洁版主页的滚动容器是自绘的
  `homepage/infoflow/view/InfoFlowScrollView`（`extends FrameLayout`，不是原生 `ScrollView`），
  回弹走 `View#overScrollBy` 那条路。开启后在容器**首次 `onLayout`** 时做两件事：

  - `overScrollMode = OVER_SCROLL_NEVER` —— 框架层不再产生边缘光晕与 Android 12+ 的过冲效果；
  - `mOverscrollDistance` / `mOverflingDistance` 置 0 —— 容器传给 `overScrollBy` 的允许越界
    距离，这是回弹幅度的直接来源。

  两道保险都做：前者管框架效果，后者管这个 View 自己的越界量。同一容器类在信息流页也复用，
  那里按 `mIsSimpleHome` 区分、保留原生手感（字段读不到时不做区分 —— 宁可生效也不静默失效）。

### 说明

- 默认关闭。拿掉的是宿主的边界手感反馈，属于「削弱宿主原有行为」的一类，按项目约定保守起步；
  设置页详情的「注意事项」里也写明了这一点。

## 1.15.6 (versionCode 45) —— 「主页快捷方式行数」新增 10 行档

### 新增

- **「主页快捷方式行数」增加「最多 10 行」选项**（原为 3 / 4 / 5 行）。机制完全不变：
  上限 = `行数 × 每行个数 − 1`，5 列时各行对应 **3 行 14 个 / 4 行 19 个 / 5 行 24 个 /
  10 行 49 个**站点格。站点不足上限时按实际数量铺，不会补空格子。

### 说明

- ⚠ 10 行时格子数最多 49 个（5 行的两倍多），内容高度远超一屏、**必定要滚动**。宿主是自绘
  网格、没有 view 复用，绘制成本随行数线性上升 —— 行数越高，1.15.5 优化掉的那部分 hook
  开销占比越小，绘制本身越容易成为瓶颈。这是功能的固有代价，设置页详情文案里也已标注。
- 本条由酷安用户 **@酩安小煸** 反馈提出。**行数上限是可选功能，没有这个需求就不用打开**
  （默认就是关闭的，不开时主页快捷方式保持宿主原样）。

## 1.15.5 (versionCode 44) —— 修「主页快捷方式多行时滑动掉帧」

### 修复

- **主页快捷方式设成 5 行后，上下滑动掉帧**。症状本身就把范围锁死在布局热路径上：关掉开关不卡；
  多行时主页内容变高、需要滚动才掉帧，而 3 行 / 4 行一屏放得下、不滚动，所以几乎察觉不到。
  滚动会反复触发宿主 `BrowserQuickLinksPage$QuickLinksPanel#onLayout`，而本功能的 after
  回调每次都要完整跑一遍 —— 三处开销正好叠在这条路径上：

  | # | 开销 | 为什么是问题 |
  |---|---|---|
  | 1 | 跨进程读 | 开关判断走 `Config.on()`，本地快照过期时（1s TTL）执行 `handle().all` —— 一次跨进程往返压在帧内 |
  | 2 | 反射无缓存 | 一次回调读 7 个字段；`Xp.field` 当时每次都重新 `getDeclaredField` + `setAccessible(true)`，字段落在父类时还要白构造一轮 `NoSuchFieldException` |
  | 3 | 参数快照 | hook 适配器对「纯 after」的回调也照做一遍数组拷贝 + 逐元素比较 |

- **三处都已处理**：

  - 新增 `Config.onCached(key)` 与 `Feature.onCached()`：**只读本地快照，绝不发起跨进程刷新**。
    布局热路径用它；快照的新鲜度交给低频调用点 —— `cap()`（即 `getShowSiteCount`，每次数据
    更新才走一次）照常走 `on()`，顺手把快照刷新上来。
  - `Xp` 增加字段查找缓存：按「运行时类 + 字段名」缓存 `Field`，「找不到」也用哨兵缓存，
    `setAccessible(true)` 随之只做一次。宿主换版后走新的 ClassLoader，缓存自然失效。
  - `SafeHook.intercept`：`before == null` 的纯 after hook 跳过 `args.copyOf()` 与 `contentEquals`。

- **另加一条零开销快速路径**：`realignMoreCell` 动手前先看 `panel.childCount`，站点格没超过
  原生 9 个（`NATIVE_CHILD_COUNT = 10`，即 9 个站点格 + 1 个「更多」格）时，宿主写死的位置 9
  本来就是对的，直接放行。开关关着、或站点本来就不多的用户，这个回调里一个反射都不做。

- 顺带修正 1.15.2 KDoc 里一处不准确的说法：「原生场景算出来和宿主一致、天然幂等」只在
  `childCount` 恰好为 10 时成立。

### 说明

- 本次优化针对的是**模块自身的开销**。多行本身的绘制成本（格子从原生 9 个涨到最多 24 个，
  宿主是自绘网格且无 view 复用）属于这个功能的固有代价，未做改动。
- 本条由酷安用户 **@酩安小煸** 反馈提出（他反馈了「快捷方式解限后滑动明显掉帧」）。
...
**功能、开关键、hook 逻辑、规则引擎一行未动**，只动界面层。

- **浮层位置太靠下、内容与按钮被挡**：详情 / 单选 / 确认三个浮层都是自绘的「贴底大卡片」，
  铺在主界面 Scaffold 的**内容区**里 —— 而底部导航栏是 Scaffold 后画上去的，
  于是卡片底部被底栏盖住：正文看不全、关闭按钮点不到。
  原先只加了 `navigationBarsPadding()`（只避开系统导航条），**没算上底栏这一层**。
  现在把底栏高度（`innerPadding.calculateBottomPadding()`）传给浮层，与系统导航条**取大者**
  （不是相加 —— miuix 导航条自身已带导航条内边距，相加会多留一层空白），
  底部再加一层页面边距，卡片不再贴着底栏。
- **卡片整体没限高**：以前只给正文限了 320dp，卡片本身可以无限长，屏幕一矮就顶出屏幕外，
  按钮跟着被推到看不见的地方。现在浮层用 `BoxWithConstraints` 拿到屏幕高，
  卡片限高为「屏幕高 − 底部让位 − 顶部留白」；中段（详情正文 / 选项列表）用
  `weight(1f, fill = false)` 跟随剩余空间压缩并内部滚动 ——
  标题与底部按钮**始终在屏幕内**，屏幕够高时卡片仍收着长（不撑满）。
- 同类问题一并覆盖：规则管理页的「从 URL 导入」与「规则明细」两个浮层、风险开关的确认浮层。

### 说明

- 新增 `ui/component/DialogInsets.kt`：底部让位与卡片限高两处计算集中在这里，五个浮层共用，
  下次再加浮层直接引用，不用再各自算一遍。
- `ui/CatalogComponents.kt` 的 `DetailBody` 增加 `modifier` 参数（原来固定限高 320dp、
  不参与卡片的高度分配）。
